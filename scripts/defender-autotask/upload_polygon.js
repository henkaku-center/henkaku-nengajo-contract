...
async function uploadCode(autotaskId, apiKey, apiSecret) {
  ...
  await client.updateCodeFromFolder(autotaskId, './build/relay_polygon')
}

async function main() {
  require('dotenv').config()
  const { TEAM_API_KEY_MAIN: apiKey, TEAM_API_SECRET_MAIN: apiSecret, AUTOTASK_ID_MAIN: autotaskId } = process.env
