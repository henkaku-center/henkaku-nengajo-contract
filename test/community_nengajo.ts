...
const calcRequiredHenkakuForRegister: (params: {
  ...
  address: Signer
  maxSupply: number
}) => Promise<bigint> = async ({ NengajoContract, address, maxSupply }) => {
  const registeredCount = (await NengajoContract.retrieveRegisteredNengajoes(await address.getAddress())).reduce(
    (sum, current) => {
      return Number(sum) + Number(current.maxSupply)
    },
    0
  )
  let amount: bigint = _calcRequiredHenkakuForRegister(registeredCount + maxSupply) - (_calcRequiredHenkakuForRegister(registeredCount))
  return amount
}

const _calcRequiredHenkakuForRegister: (maxSupply: number) => bigint = (maxSupply) => {
  let amount
  if (maxSupply <= 10) {
    amount = hardhatEthers.parseEther('10')
  } else if (10 < maxSupply && maxSupply < 101) {
    amount = hardhatEthers.parseEther(`${maxSupply * 5 - 40}`)
  } else {
    amount = hardhatEthers.parseEther(`${maxSupply * 10 - 540}`)
  }
  return amount
}
...
describe('RegisterNengajo', () => {
  ...
  let user2: Signer
  let user3: Signer
  let externalUser1: Signer

  before(async () => {
    [deployer, creator, user1, user2, user3, externalUser1] = await hardhatEthers.getSigners()
    HenkakuTokenContract = await deployAndDistributeHenkakuV2({
      deployer,
      addresses: [await creator.getAddress(), await user1.getAddress(), await user2.getAddress(), await user3.getAddress(), await deployer.getAddress()],
      amount: hardhatEthers.parseEther('1000'),
    })
  ...
  it('register creative', async () => {
    ...
    expect(getAllRegisteredNengajos[0].creator).to.equal(hardhatEthers.ZeroAddress)
    ...
    expect(getRegisteredNengajo.creator).to.equal(hardhatEthers.ZeroAddress)
    expect(getRegisteredNengajo.maxSupply).to.equal(0)

    let henkakuBalance
    henkakuBalance = await HenkakuTokenContract.balanceOf(await creator.getAddress())
    expect(henkakuBalance).to.equal(hardhatEthers.parseEther('1000'))

    await HenkakuTokenContract.connect(creator).approve(NengajoContract.getAddress(), hardhatEthers.parseEther('200'))
    ...
    henkakuBalance = await HenkakuTokenContract.balanceOf(await creator.getAddress())
    expect(henkakuBalance).to.equal(hardhatEthers.parseEther('990'))
  ...
  it('check 1Henkaku transfered', async () => {
    const henkakuBalance = await HenkakuTokenContract.balanceOf(await creator.getAddress())
    expect(henkakuBalance).to.equal(hardhatEthers.parseEther('990'))
  })
  ...
  it('Check expected henkaku token amount is transfered', async () => {
    const henkakuBalanceBefore = await HenkakuTokenContract.balanceOf(await creator.getAddress())
    ...
    const henkakuBalanceAfter = await HenkakuTokenContract.balanceOf(await creator.getAddress())
    expect(henkakuBalanceAfter).to.equal(henkakuBalanceBefore - expectedHenkakuAmount)
  })
})
...
describe('MintNengajo', () => {
  ...
  before(async () => {
    ...
    await HenkakuTokenContract.connect(creator).approve(NengajoContract.getAddress(), hardhatEthers.parseEther('1000'))
...
describe('CheckMintable', () => {
  ...
  before(async () => {
    [deployer, creator, user1, user2, user3] = await hardhatEthers.getSigners()
    HenkakuTokenContract = await deployAndDistributeHenkakuV2({
      deployer,
      addresses: [await creator.getAddress(), await user1.getAddress(), await user2.getAddress(), await deployer.getAddress()],
      amount: hardhatEthers.parseEther('100'),
    })
    const NengajoFactory = (await hardhatEthers.getContractFactory('Nengajo'))
    NengajoContract = (await upgrades.deployProxy(
      NengajoFactory,
    ...
    HenkakuTokenContract = await deployAndDistributeHenkakuV2({
      deployer,
      addresses: [await creator.getAddress(), await user1.getAddress(), await user2.getAddress()],
      amount: hardhatEthers.parseEther('100'),
    })
  })
  ...
  it('check pool address changed', async () => {
    ...
    expect(pool).to.equal(await deployer.getAddress())

    await (await NengajoContract.connect(deployer).changeHenkakuPool(await user1.getAddress())).wait()

    pool = await NengajoContract.henkakuPoolWallet()
    expect(pool).to.equal(await user1.getAddress())
  })
  ...
  it('check pool address cannot be changed except by admin', async () => {
    ...
    expect(await NengajoContract.connect(user1).changeHenkakuPool(await user2.getAddress())).to.revertedWith(
...
describe('check calculation of Henkaku amount', () => {
  ...
  it('register total 11', async () => {
    ...
    henkakuBalance = await HenkakuTokenContract.balanceOf(await creator.getAddress())
    expect(henkakuBalance).to.equal(hardhatEthers.parseEther('985'))
  })
  ...
  it('register total 12', async () => {
    ...
    henkakuBalance = await HenkakuTokenContract.balanceOf(await creator.getAddress())
    expect(henkakuBalance).to.equal(hardhatEthers.parseEther('980'))
  })
  ...
  it('register total 100', async () => {
    ...
    henkakuBalance = await HenkakuTokenContract.balanceOf(await creator.getAddress())
    expect(henkakuBalance).to.equal(hardhatEthers.parseEther('540'))
  })
  ...
  it('register total 101', async () => {
    ...
    henkakuBalance = await HenkakuTokenContract.balanceOf(await creator.getAddress())
    expect(henkakuBalance).to.equal(hardhatEthers.parseEther('530'))
  })
  ...
  it('register total 102', async () => {
    ...
    henkakuBalance = await HenkakuTokenContract.balanceOf(await creator.getAddress())
    expect(henkakuBalance).to.equal(hardhatEthers.parseEther('520'))
  })
})
